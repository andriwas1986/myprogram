...
// --- PROSES UPLOAD ---
if (isset($_FILES["fotoGadik"])) {
    $file = $_FILES["fotoGadik"];
    $imageFileType = strtolower(pathinfo($file["name"], PATHINFO_EXTENSION));

    // Buat nama file yang unik
    $unique_name = uniqid('gadik_', true) . '.' . $imageFileType;
    ...
    if (move_uploaded_file($file["tmp_name"], $target_file)) {
        ...
        $file_url = UPLOAD_URL . 'gadik/' . $unique_name;
        
        echo json_encode(["success" => true, "url" => $file_url]);
    } else {
        error_log("Upload Gadik Gagal: " . print_r(error_get_last(), true));
        echo json_encode(["success" => false, "message" => "Terjadi kesalahan saat mengunggah file ke server."]);
    }

} else {
    echo json_encode(["success" => false, "message" => "Tidak ada file 'fotoGadik' yang diterima."]);
}
?>