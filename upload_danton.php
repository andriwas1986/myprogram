...
// --- PROSES UPLOAD ---
if (isset($_FILES["fotoDanton"])) {
    $file = $_FILES["fotoDanton"];
    $imageFileType = strtolower(pathinfo($file["name"], PATHINFO_EXTENSION));

    // Buat nama file yang unik
    $unique_name = uniqid('danton_', true) . '.' . $imageFileType;
    ...
    if (move_uploaded_file($file["tmp_name"], $target_file)) {
        ...
        $file_url = UPLOAD_URL . 'danton/' . $unique_name;
        
        echo json_encode(["success" => true, "url" => $file_url]);
    } else {
        error_log("Upload Danton Gagal: " . print_r(error_get_last(), true));
        echo json_encode(["success" => false, "message" => "Terjadi kesalahan saat mengunggah file ke server."]);
    }

} else {
    echo json_encode(["success" => false, "message" => "Tidak ada file 'fotoDanton' yang diterima."]);
}
?>